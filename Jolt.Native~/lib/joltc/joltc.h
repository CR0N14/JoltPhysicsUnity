...
typedef struct JPH_StateRecorder_Procs {
	void(JPH_API_CALL* ReadBytes)(void* userData,
		void* outData,
		size_t inNumBytes
		);
	...
	void(JPH_API_CALL* WriteBytes)(void* userData,
		const void* inData,
		size_t inNumBytes
		);
...
JPH_CAPI void JPH_StateRecorder_Destroy(JPH_StateRecorder* listener);
...
JPH_CAPI void JPH_BodyInterface_GetCenterOfMassTransform(JPH_BodyInterface* bodyInterface, JPH_BodyID bodyId, JPH_RMatrix4x4* result);
